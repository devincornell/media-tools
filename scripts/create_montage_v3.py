...
if __name__ == '__main__':
    ...
    video_paths = mdir.all_video_paths()

    print(f"  Creating montage: {len(video_paths)} videos from {settings.montage_source_path} -> {settings.montage_target_path}")
    result = mediatools.ffmpeg.create_montage(
        video_files=video_paths,
        output_filename=settings.montage_target_path,
        clip_ratio=10,
        clip_duration=0.75,
        random_seed=0,
        width=1920,
